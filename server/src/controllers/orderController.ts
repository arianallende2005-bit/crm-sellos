...
export const getOrders = async (req: Request, res: Response) => {
    try {
        ...
        if (user.role === 'cliente') {
            ...
            // Show archived only for 1 month logic will be handled or filtered
            // For MVP, just show all or filter by flag
    ...
    try {
        ...
        const order = await prisma.order.findUnique({
            ...
            include: {
                ...
                history: {
                    include: { changer: { select: { username: true } } } // Changer might need map in schema or check generated
                }
            },
        });
        ...
        const updatedOrder = await prisma.$transaction(async (tx: Prisma.TransactionClient) => {
            const order = await tx.order.update({
                ...
                data: {
                    ...
                    // If status is 5 (Enviado), typically we don't archive yet, cron does it by 30 days
                },
            });
